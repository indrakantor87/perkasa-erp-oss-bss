-- Jalankan file ini setelah `xampp_review_sample_import_wave_1c_sales.sql`.
-- Transform ini khusus untuk batch sample coverage dan marketing activity.
...
SELECT
  ob.id,
  COALESCE(NULLIF(TRIM(s.area_code), ''), CONCAT('PSB-AREA-', LPAD(s.id, 6, '0'))),
...
WHERE s.batch_id = @coverage_batch_id
  ...
  AND NOT EXISTS (
    ...
    WHERE a.area_code = COALESCE(NULLIF(TRIM(s.area_code), ''), CONCAT('PSB-AREA-', LPAD(s.id, 6, '0')))
  );

UPDATE staging_legacy_sales_coverage_records s
JOIN sales_covered_areas a
  ON a.area_code = COALESCE(NULLIF(TRIM(s.area_code), ''), CONCAT('PSB-AREA-', LPAD(s.id, 6, '0')))
