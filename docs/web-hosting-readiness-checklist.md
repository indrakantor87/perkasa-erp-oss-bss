...
- Helper `npm run capture:server-proof-pack -- ...` kini tersedia untuk menjalankan paket bukti server-side (`reverse proxy` -> `runtime` -> `runtime report` -> `evidence collector`) dalam satu command yang lebih aman untuk operator hari-H.
- Helper `capture:server-proof-pack` kini juga mendukung `--stamp` dan `--output-dir` agar setiap rehearsal / hari-H menghasilkan bundle artefak bertimestamp yang siap audit tanpa menimpa file sebelumnya.
- Helper `npm run evaluate:server-readiness -- ...` kini tersedia untuk membaca bundle `docs/go-live/` dan mengeluarkan keputusan teknis `ready`, `partial`, atau `rollback-recommended` sebelum sign-off PIC.
...
- Lembar [web-server-operator-handoff.md](file:///d:/trae_projects/perkasa-erp-oss-bss/docs/web-server-operator-handoff.md) kini tersedia sebagai handoff paling ringkas untuk operator host nyata agar command utama, output wajib, rollback, dan sign-off final tidak tercecer.
