...
| `SALES_MARKETING` | Pemasaran dan Pelayanan / Penjualan | dashboard, daily activity, list kerja, sales, customers, monitoring support dan inventory | flow lead dan onboarding awal sudah hidup, ditopang `List Kerja` lintas lead/customer/coverage/order | parity `marketing-activities` dan ritme campaign harian masih belum lengkap walau CTA misleading `Import Center` sudah dibersihkan | `PARTIAL` | Gelombang 2 |
| `CS_OPERATOR` | Pemasaran dan Pelayanan / CS | dashboard, daily activity, list kerja, sales, customers, support, inventory | smoke UAT 2026-07-13 lulus: login, landing worklist, dan perpindahan lintas domain dasar berjalan | write-side end-to-end dan pembuktian ritme kerja operator masih perlu diperdalam | `PARTIAL` | Gelombang 2 |
| `CS_ADMIN` | Pemasaran dan Pelayanan / Admin CS | dashboard, daily activity, list kerja, sales, customers, support, inventory, workspace supervisor CS | approval daily activity, queue koreksi, restore, dan backlog risiko supervisor sudah mulai nyata | query supervisor masih fallback `Column 'status' in field list is ambiguous`, jadi bukti UAT berisiko belum final | `PARTIAL` | Gelombang 2 |
...
| `DISMANTLE_OPERATOR` | Pemasaran dan Pelayanan / Dismantle | dashboard, daily activity, list kerja, support | smoke UAT 2026-07-13 lulus: login, landing `/support/dismantle`, queue aktif, dan guard mikro-role tampak presisi | bukti write-side close/reopen masih perlu diformalisasi untuk naik ke pilot | `PARTIAL` | Gelombang 2 |
