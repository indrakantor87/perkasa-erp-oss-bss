...
export default async function SalesSurveysPage() {
  const session = await requireSession()
  if (!canAccessPath(session.role, '/sales/surveys')) {
    redirect('/dashboard')
  }
  ...
  if (reviewDbReady) {
    try {
      const list = await getSalesDomainListPageData('surveys', source, session.role)
      isLoading = list.isLoading
      errorMessage = list.errorMessage
      rows = list.rows
    } catch (err) {
      errorMessage = err instanceof Error ? err.message : 'Gagal memuat data surveys.'
      isLoading = false
    }
  }
