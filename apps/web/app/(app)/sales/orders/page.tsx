...
export default async function SalesOrdersPage() {
  ...
  if (!canAccessPath(session.role, '/sales/orders')) {
    redirect('/dashboard')
  }
  ...
  if (reviewDbReady) {
    try {
      const list = await getSalesDomainListPageData('orders', source, session.role)
      isLoading = list.isLoading
      errorMessage = list.errorMessage
      rows = list.rows
    } catch (err) {
      errorMessage = err instanceof Error ? err.message : 'Gagal memuat data orders.'
      isLoading = false
    }
  }
