...
export default async function SalesQuotationsPage() {
  ...
  if (!canAccessPath(session.role, '/sales/quotations')) {
    redirect('/dashboard')
  }
  ...
  if (reviewDbReady) {
    try {
      const list = await getSalesDomainListPageData('quotations', source, session.role)
      isLoading = list.isLoading
      errorMessage = list.errorMessage
      rows = list.rows
    } catch (err) {
      errorMessage = err instanceof Error ? err.message : 'Gagal memuat data quotations.'
      isLoading = false
    }
  }
