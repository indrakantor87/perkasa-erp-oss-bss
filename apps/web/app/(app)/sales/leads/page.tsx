...
export default async function SalesLeadsPage() {
  ...
  if (!canAccessPath(session.role, '/sales/leads')) {
    redirect('/dashboard')
  }
  ...
  if (reviewDbReady) {
    try {
      const list = await getSalesDomainListPageData('leads', source, session.role)
      isLoading = list.isLoading
      errorMessage = list.errorMessage
      rows = list.rows
    } catch (err) {
      errorMessage = err instanceof Error ? err.message : 'Gagal memuat data leads.'
      isLoading = false
    }
  }
