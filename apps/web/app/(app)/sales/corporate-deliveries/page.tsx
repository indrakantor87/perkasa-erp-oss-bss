...
export default async function SalesCorporateDeliveriesPage() {
  const session = await requireSession()
  if (!canAccessPath(session.role, '/sales/corporate-deliveries')) {
    redirect('/dashboard')
  }
  ...
  if (reviewDbReady) {
    try {
      const list = await getSalesDomainListPageData('corporate-deliveries', source, session.role)
      isLoading = list.isLoading
      errorMessage = list.errorMessage
      rows = list.rows
    } catch (err) {
      errorMessage = err instanceof Error ? err.message : 'Gagal memuat data corporate deliveries.'
      isLoading = false
    }
  }
  ...
  return (
    <SalesDomainListPage
      entityKey="corporate-deliveries"
      role={session.role}
