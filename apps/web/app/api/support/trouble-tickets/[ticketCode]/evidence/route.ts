  ...
  try {
    ...
    if (contentType.toLowerCase().includes('multipart/form-data')) {
      ...
      if (form) {
        ...
        const safeTicketCode = ticketCode.replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 40) || 'ticket'
        const storageFilename = `tt_${safeTicketCode}_${stamp}_${Math.random().toString(36).slice(2, 8)}${ext}`
        ...
        let mTroubleIdRaw: number | null = null
        try {
          const ttRows = await runReviewDbQuery<any>(`SELECT id FROM support_trouble_tickets WHERE UPPER(ticket_code) = ? LIMIT 1`, [ticketCode])
          if (ttRows.length > 0) mTroubleIdRaw = Number(ttRows[0].id ?? 0) || null
        } catch (_) { }
        const mTicketId = mTroubleIdRaw ? await resolveTicketIdByTroubleTicketCode(mTroubleIdRaw) : null
        if (!mTicketId) {
          return NextResponse.json({ error: 'FORBIDDEN' }, { status: 403 })
        }

        try {
          const statusRes = await runReviewDbQuery<any>(`SELECT status FROM support_trouble_tickets WHERE ticket_code = ?`, [ticketCode])
    ...
    let troubleIdRaw: number | null = null
    try {
      const ttRows = await runReviewDbQuery<any>(`SELECT id FROM support_trouble_tickets WHERE UPPER(ticket_code) = ? LIMIT 1`, [ticketCode])
      if (ttRows.length > 0) troubleIdRaw = Number(ttRows[0].id ?? 0) || null
    } catch (_) { }
    const ticketId = troubleIdRaw ? await resolveTicketIdByTroubleTicketCode(troubleIdRaw) : null
    if (!ticketId) {
      return NextResponse.json({ error: 'FORBIDDEN' }, { status: 403 })
    }
