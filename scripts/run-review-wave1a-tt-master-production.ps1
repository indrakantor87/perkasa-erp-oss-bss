param(
  [ValidateSet('Wave1ATTMasterProductionOnly')]
  [string]$Mode = 'Wave1ATTMasterProductionOnly',
...
$jsonDirectory = if ($JsonDir) {
  ...
  Join-Path $repoRoot 'production-data\web-psb-wave1a-tt-master'
}

if (-not (Test-Path $jsonDirectory)) {
  throw "Folder JSON production TroubleTicketMaster tidak ditemukan: $jsonDirectory"
}

$generatedSqlFile = if ($GeneratedSqlPath) {
  $GeneratedSqlPath
} else {
  Join-Path $repoRoot 'database\generated\web_psb_wave1a_tt_master_production_loader.sql'
}

$generatorScript = Join-Path $repoRoot 'scripts\generate-wave1a-tt-master-production-loader.mjs'
...
if ($LASTEXITCODE -ne 0) {
  throw "Generator SQL Wave 1A TroubleTicketMaster production gagal dijalankan (exit code $LASTEXITCODE)."
}

$sequence = @(
  'database/xampp_review_patch_wave_1a_existing_review_db.sql',
  'database/xampp_review_patch_wave1a_tt_master_existing_review_db.sql',
  $generatedSqlFile,
  'database/xampp_review_transform_wave1a_tt_master_production.sql'
)
...
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave1a_tt_master_production_review_queries.sql')
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave1a_tt_master_production_assertions.sql')

Write-Host ''
Write-Host 'Runner Wave 1A TroubleTicketMaster production selesai.'
