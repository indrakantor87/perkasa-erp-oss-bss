param(
  [ValidateSet('Wave1WhatsappTemplateProductionOnly')]
  [string]$Mode = 'Wave1WhatsappTemplateProductionOnly',
...
$jsonDirectory = if ($JsonDir) {
  ...
  Join-Path $repoRoot 'production-data\web-psb-wave1-whatsapp-template'
}

if (-not (Test-Path $jsonDirectory)) {
  throw "Folder JSON production WhatsappTemplate tidak ditemukan: $jsonDirectory"
}

$generatedSqlFile = if ($GeneratedSqlPath) {
  $GeneratedSqlPath
} else {
  Join-Path $repoRoot 'database\generated\web_psb_wave1_whatsapp_template_production_loader.sql'
}

$generatorScript = Join-Path $repoRoot 'scripts\generate-wave1-whatsapp-template-production-loader.mjs'
...
if ($LASTEXITCODE -ne 0) {
  throw "Generator SQL Wave 1 WhatsappTemplate production gagal dijalankan (exit code $LASTEXITCODE)."
}

$sequence = @(
  'database/xampp_review_patch_wave1_whatsapp_template_existing_review_db.sql',
  $generatedSqlFile,
  'database/xampp_review_transform_wave1_whatsapp_template_production.sql'
)
...
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave1_whatsapp_template_production_review_queries.sql')
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave1_whatsapp_template_production_assertions.sql')

Write-Host ''
Write-Host 'Runner Wave 1 WhatsappTemplate production selesai.'
