param(
  [ValidateSet('Wave1BTicketProductionOnly')]
  [string]$Mode = 'Wave1BTicketProductionOnly',
...
$jsonDirectory = if ($JsonDir) {
  ...
  Join-Path $repoRoot 'production-data\web-psb-wave1b'
}

if (-not (Test-Path $jsonDirectory)) {
  throw "Folder JSON production tidak ditemukan: $jsonDirectory"
}

$generatedSqlFile = if ($GeneratedSqlPath) {
  $GeneratedSqlPath
} else {
  Join-Path $repoRoot 'database\generated\web_psb_wave1b_ticket_production_loader.sql'
}

$generatorScript = Join-Path $repoRoot 'scripts\generate-wave1b-ticket-production-loader.mjs'
...
if ($LASTEXITCODE -ne 0) {
  throw "Generator SQL Wave 1B Ticket production gagal dijalankan (exit code $LASTEXITCODE)."
}
...
$sequence = @(
  ...
  $generatedSqlFile,
  'database/xampp_review_transform_wave1b_ticket_production.sql'
)
...
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave1b_ticket_production_review_queries.sql')
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave1b_ticket_production_assertions.sql')

Write-Host ''
Write-Host 'Runner Wave 1B Ticket production selesai.'
