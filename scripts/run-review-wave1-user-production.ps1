param(
  [ValidateSet('Wave1UserProductionOnly')]
  [string]$Mode = 'Wave1UserProductionOnly',
...
$jsonDirectory = if ($JsonDir) {
  ...
  Join-Path $repoRoot 'production-data\web-psb-wave1-user'
}

if (-not (Test-Path $jsonDirectory)) {
  throw "Folder JSON production User tidak ditemukan: $jsonDirectory"
}

$generatedSqlFile = if ($GeneratedSqlPath) {
  $GeneratedSqlPath
} else {
  Join-Path $repoRoot 'database\generated\web_psb_wave1_user_production_loader.sql'
}

$generatorScript = Join-Path $repoRoot 'scripts\generate-wave1-user-production-loader.mjs'
...
if ($LASTEXITCODE -ne 0) {
  throw "Generator SQL Wave 1 User production gagal dijalankan (exit code $LASTEXITCODE)."
}

$sequence = @(
  'database/xampp_review_patch_wave_1a_existing_review_db.sql',
  $generatedSqlFile,
  'database/xampp_review_transform_wave1_user_production.sql'
)
...
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave1_user_production_review_queries.sql')
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave1_user_production_assertions.sql')

Write-Host ''
Write-Host 'Runner Wave 1 User production selesai.'
