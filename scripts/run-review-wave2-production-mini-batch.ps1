param(
  [ValidateSet('Wave2ProductionMiniBatchOnly')]
  [string]$Mode = 'Wave2ProductionMiniBatchOnly',
...
$jsonDirectory = if ($JsonDir) {
  ...
  Join-Path $repoRoot 'production-data\web-psb-wave2'
}
...
$generatedSqlFile = if ($GeneratedSqlPath) {
  ...
  Join-Path $repoRoot 'database\generated\web_psb_wave2_production_loader.sql'
}

$generatorScript = Join-Path $repoRoot 'scripts\generate-wave2-production-loader.mjs'
...
if ($LASTEXITCODE -ne 0) {
  throw "Generator SQL Wave 2 gagal dijalankan (exit code $LASTEXITCODE)."
}
...
$sequence = @(
  ...
  $generatedSqlFile,
  'database/xampp_review_transform_wave2_sales.sql',
  'database/xampp_review_transform_wave2_odp_ttsla.sql',
...
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave2_production_review_queries.sql')

Write-Host ''
Write-Host 'Runner Wave 2 production mini-batch selesai.'
