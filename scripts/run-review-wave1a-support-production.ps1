param(
  [ValidateSet('Wave1ASupportProductionOnly')]
  [string]$Mode = 'Wave1ASupportProductionOnly',
...
$jsonDirectory = if ($JsonDir) {
  ...
  Join-Path $repoRoot 'production-data\web-psb-wave1a-support'
}

if (-not (Test-Path $jsonDirectory)) {
  throw "Folder JSON production support tidak ditemukan: $jsonDirectory"
}

$generatedSqlFile = if ($GeneratedSqlPath) {
  $GeneratedSqlPath
} else {
  Join-Path $repoRoot 'database\generated\web_psb_wave1a_support_production_loader.sql'
}

$generatorScript = Join-Path $repoRoot 'scripts\generate-wave1a-support-production-loader.mjs'
...
if ($LASTEXITCODE -ne 0) {
  throw "Generator SQL Wave 1A support production gagal dijalankan (exit code $LASTEXITCODE)."
}

$sequence = @(
  'database/xampp_review_patch_wave_1a_existing_review_db.sql',
  $generatedSqlFile,
  'database/xampp_review_transform_wave1a_support_production.sql'
)
...
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave1a_support_production_review_queries.sql')
Invoke-ReviewFile -MysqlExe $mysqlExe -MysqlArgs $mysqlArgs -FilePath (Join-Path $repoRoot 'database/xampp_review_wave1a_support_production_assertions.sql')

Write-Host ''
Write-Host 'Runner Wave 1A support production selesai.'
